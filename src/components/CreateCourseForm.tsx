...
const CreateCourseForm = (props: Props) => {
    ...
    const {mutate: createChapters, isPending} = useMutation({
        mutationFn: async ({title, units, src}: Input) => {
            const response = await axios.post('/api/course/createChapters', {title, units, src})
    ...
    function onSubmit(data: Input) {
        ...
        if (data.src === "") {
            data.src = "https://res.cloudinary.com/dcublke0e/image/upload/v1705697368/yp2ffvhs1pzeuje23dlj.png"
        }
        createChapters(data, {
            onSuccess: ({ course_id }) => {
                toast({
        ...
        <div className="w-full">
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="w-full mt-4">
                    ...
                    <Button disabled={isPending} type="submit" className="w-full mt-6 flex justify-between" size="lg">
                        <div></div>
                        <div>Let&#39;s Go!</div>
